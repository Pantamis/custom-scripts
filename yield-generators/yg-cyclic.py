...
class YieldGeneratorCyclicEnhanced(YieldGeneratorBasic):
    ...
    def select_input_mixdepth(self, available, offer, amount):
        """We select the mixdepth to maximize the largest interval
        in cyclic order of non-available mixdepths by choosing the first
        available after the largest such interval. The biggest UTXOs
        will stay in a bulk of few mixdepths which maximize the size of
        maker orders and remove dependancy to mixdepth numbering.
        In case where two or more intervals are the largest, we choose to
        maximize the balance of the receiving mixdepth to increase the
        probability of concentrating the chosen bulk with next orders.
        If the order can be filled by any mixdepth, we choose the one that
        have higher chances to be the oldest one used to consolidate funds.
        """
        # We first handles the case where all mixdepth can be used to fill the order
        if len(available)==self.wallet_service.mixdepth + 1:
            # We search the mixdepth which has more funds than other
            rank = max(available, key = available.get)
            # After many coinjoins, it will be more likely that most of the funds will
            # be in one or two mixdepths because of the way we choose input ones.
            # The balances of mixdepths are expected to be increasing in cyclic order
            # until the mixdepth where most of the funds are. They fall back to start
            # and the cycle of mixdepth continue. Ideally we want to consolidate the
            # mixdepth where the increasing trend start again. It can only be in
            # one of the three mixdepths after the maximum one. We expect it to have the
            # lowest balance. So we select the one with less fund of the 3.
            return min([(rank+1)%(self.wallet_service.mixdepth + 1),
                        (rank+2)%(self.wallet_service.mixdepth + 1),
                        (rank+3)%(self.wallet_service.mixdepth + 1)],
                       key = available.get)
        ...
        return available[max(candidates, key = lambda entry: mix_balance[(available[entry]+1)%(len(available))])]
...
if __name__ == "__main__":
    ygmain(YieldGeneratorCyclicEnhanced, nickserv_password='')
